...
**注意：这里的路径需要注意的根据启动redis服务的当前目录起算，例如当前目录启动redis服务的目录是config,src和config是同级目录，日志文件在src同级别的config目录下的logs文件夹，则日志的文件的路径应该写"../config/logs/6380.log,或者写绝对路径或者通过环境变量来设置绝对路径",数据文件的路径也是类似,最后一点，当bind的ip和真实ip不一致则启动不了，并且没有日志没有提示**
...
bind 10.1.210.69   #监听地址，如果是单机多个示例可以不用修改,不改ip不能远程连接

port 6379         #监听端口，保持和配置文件名称端口一致
masterauth 654321  # 输入master的登录密码，否则从库不能连接主库
~~~
